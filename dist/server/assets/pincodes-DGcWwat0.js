...
function isPincodeSupported(pincode) {
  if (pincode === null || pincode === void 0) return false;
  const str = String(pincode).trim();
  return SUPPORTED_PINCODES.includes(str);
}
function getPincodeLocation(pincode) {
  if (pincode === null || pincode === void 0) return "";
  const str = String(pincode).trim();
  return PINCODE_LOCATION_MAP[str] || "";
}
export {
  getPincodeLocation as g,
  isPincodeSupported as i
};
